 ...
 * @author susu
 * @version 1.0.5
 * @link https://github.com/cnhongv/typecho-cos-plugin
 * @dependence 1.2.1-1.3.0
 * @date 2022-08-08
 * @update 2026-06-19
 */
...
?>
    ...
    <div class="joe_config">
      ...
      <div class="joe_config__notice">
        ...
        <ol>
          ...
          <li>插件基于腾讯云cos-php-sdk-v5开发，若发现插件不可用，请到 <a target="_blank" href="https://github.com/cnhongv/typecho-cos-plugin">GitHub发布地址</a> 检查是否有更新，或者提交Issues<br></li>
  ...
  public static function attachmentHandle($content)
  {
    ...
    $opt = Options::alloc()->plugin(pluginName);
    $cosClient = self::CosInit();
    #适配新版Typecho核心：$content可能是Config对象（直接传attachment）或旧版数组格式
    $path = is_array($content) ? ($content['attachment']->path ?? null) : ($content->path ?? null);
    if (empty($path)) {
      return false;
    }
    if ($opt->sign == 'open') {
  ...
  public static function attachmentDataHandle($content)
  {
    ...
    $opt = Options::alloc()->plugin(pluginName);
    $cosClient = self::CosInit();
    #适配新旧版Typecho核心：$content可能是Config对象或数组格式
    $path = is_array($content) ? ($content['attachment']->path ?? null) : ($content->path ?? null);
    if (empty($path)) {
      return false;
    }
